    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if not os.path.exists(image_path):
            ...
            for ext in [".png", ".jpg", ".jpeg", ".PNG", ".JPEG"]:
                candidate = os.path.join(images_folder, stem + ext)
        ...
        features = torch.load(os.path.join(features_folder, image_name.split('.')[0] + ".pt"), weights_only=False) if features_folder is not None else None
        masks = torch.load(os.path.join(masks_folder, image_name.split('.')[0] + ".pt"), weights_only=False) if masks_folder is not None else None
        mask_scales = torch.load(os.path.join(mask_scale_folder, image_name.split('.')[0] + ".pt"), weights_only=False) if mask_scale_folder is not None else None
